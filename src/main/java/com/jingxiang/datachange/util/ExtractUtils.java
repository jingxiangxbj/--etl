...
public class ExtractUtils {
    ...
    public void start(String status, String stage, String detail, int id, String studentInfo, RestHighLevelClient client) throws IOException {
...
//        createIndex(client,indexName);
    ...
    private String getIndexName(Long millis) {
        ...
        String indexName = "extract-" + format.format(millis);
        return indexName;
    }
    ...
    private void printLog(RestHighLevelClient client) throws IOException {
        try {
            ...
            map.put("id", id);
            map.put("studentInfo", studentInfo.toString());
            String indexName = this.getIndexName(callTime);
            Response response = client.getLowLevelClient().performRequest("HEAD", "/extract");

//        int statusCode = response.getStatusLine().getStatusCode();
//        if (statusCode == 404) {
//            //触发创建索引的逻辑
//            boolean statu = this.createIndex(client, "extract");
//            if (statu == true) {
...
//            }
//        }
            IndexRequest indexRequest = new IndexRequest("extract", "doc").source(map);
